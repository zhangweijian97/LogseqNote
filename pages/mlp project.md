...
##### 第四轮。由于以上部分需要大量时间，显然短时间内没办法继续evaluation method的实现。问一问林扬能否take这部分。顺利的话，等我完成前三轮，楚婷和林扬会完成模型的实现和测试。并且林扬会完成评估方法的实现和实验。所以这一轮，我首先需要了解他们的实验流程和结论，收集各种图片素材等。然后写实验部分的论文。
##### 第五轮，以上部分包含了整个项目的主体内。到这里我应该已经写完了相关工作，模型和实验三个部分。所以这一轮我要回顾cw3和cw3feedback。提取可用的素材，然后写introduction，conclusion，和abstract。
