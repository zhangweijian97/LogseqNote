...
## 试写
### 从文本生成 高精度 细粒度 图片的任务是非常有挑战性的。已经有一些深度生成模型做到了很不错的效果（需要再找一些材料，只有AttnGAN的相关工作提到了几个）。他们能够自动的从单词和字符中学习具有高辨别度和泛化性的图片。与之对比，GAN能学习到一个直接从单词和字符到图像像素的映射函数（需要更多材料，原因同上）。所以我们可以对生成的图片有更好的控制。
#### 深度生成模型
